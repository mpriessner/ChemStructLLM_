...
class DecodeSampler:
    ...
    def decode(self, decode_fn, batch_size, sampling_alg="greedy", device="cpu", **kwargs):
        ...
        if sampling_alg == "greedy":
            ...
            #import IPython; IPython.embed();
            print("beam")
        elif sampling_alg == "multinomial":
            output = self.multinomial_decode(decode_fn, batch_size, device)
        else:
            raise ValueError(f"Unknown sampling algorithm {sampling_alg}")
    ...
    # def multinomial_decode(self, decode_fn, batch_size, device, num_samples=10, temperature=1.0):
    #     token_ids = torch.full((1, batch_size), self.begin_token_id, dtype=torch.long, device=device)
    #     pad_mask = torch.zeros_like(token_ids, dtype=torch.bool, device=device)
        
    #     sampled_tokens = []
    #     log_probs = torch.zeros(batch_size, device=device)
        
    #     for _ in range(self.max_seq_len - 1):
    #         logits = decode_fn(token_ids, pad_mask)
