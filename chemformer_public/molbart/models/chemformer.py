...
class Chemformer:
    ...
    def __init__(
        ...
        Args:
            ...
            The config includes the following arguments:
                ...
                sampling_alg (str): Algorithm to use for sampling from the model.
                    ["beam", "greedy", "multinomial"]
        ...
        if self.config.sampling_alg == "beam" or self.config.sampling_alg == "greedy":
        ...
        elif self.config.sampling_alg == "multinomial":
            self.sampler = MultinomialSearchSampler(
                self.tokenizer,
                trainer_utils.instantiate_scorers(self.config.get("scorers")),
                util.DEFAULT_MAX_SEQ_LEN,
                device=device,
            ...
            print(f"Unknown sampling algorithm: {self.config.sampling_alg}")
    ...
    def predict(
        ...
        Args:
            ...
            sampling_alg: Which sampling algorithm to use (["beam", "multinomial"]).
        Returns:
            (sampled_smiles List[np.ndarray], log_lhs List[np.ndarray], target_smiles List[np.ndarray])
        """
        print("DEBUG: Entering Chemformer.predict()")
        print(f"DEBUG: Dataset={dataset}, sampling_alg={self.config.sampling_alg}")
        print(f"DEBUG: Config keys: {list(self.config.keys())}")
        print(f"DEBUG: Temperature: {self.config.get('temperature', 'Not set')}")
        print(f"DEBUG: Number of beams: {self.config.get('n_beams', 'Not set')}")
        
        print("predict")

        if dataloader is None:
            dataloader = self.get_dataloader(dataset)
            print(f"DEBUG: Got dataloader with {len(dataloader)} batches")
        else:
            print(f"DEBUG: Using provided dataloader")
        ...
        for batch in dataloader:
            ...
            print(f"DEBUG: Target SMILES in batch: {len(batch['target_smiles'])}")
            
            batch = self.on_device(batch)
            with torch.no_grad():
                if self.config.sampling_alg == "beam":
                    print("DEBUG: Using beam search sampling")                   
                    print("beam")                   
                    smiles_batch, log_lhs_batch = self.model.sample_molecules(
                        batch, sampling_alg="beam", return_tokenized=return_tokenized
                    )
                    print(f"DEBUG: Beam search completed, got {len(smiles_batch)} results")
                    if self.model.sampler.sample_unique:
                        smiles_batch = self.sampler.smiles_unique
                        log_lhs_batch = self.sampler.log_lhs_unique
                elif self.config.sampling_alg == "multinomial":
                    print(f"DEBUG: Using multinomial sampling with temperature={self.config.get('temperature', 1.0)}")
                    print("multinomial")
                    temperature = self.config.get('temperature', 1.0)
                    n_beams = self.config.get('n_beams', 10)
                    print(f"DEBUG: Calling model.sample_molecules with temperature={temperature}")
                    # For multinomial sampling, we call the sampler's sample_molecules method directly
                    # instead of going through the model's method
                    smiles_batch, log_lhs_batch = self.sampler.sample_molecules(
                        self.model,
                        batch, 
                        num_samples=n_beams,
                        sampling_alg="multinomial", 
                        return_tokenized=return_tokenized, 
                        temperature=temperature
                    )
                    print(f"DEBUG: Multinomial sampling completed, got {len(smiles_batch) if smiles_batch else 0} results")
                    if smiles_batch and len(smiles_batch) > 0:
                        print(f"DEBUG: First result has {len(smiles_batch[0])} predictions")
                else:
                    print(f"DEBUG: Unknown sampling algorithm: {self.config.sampling_alg}")
                    raise ValueError(f"Unknown sampling algorithm: {self.config.sampling_alg}")
            
            if isinstance(smiles_batch, (list, np.ndarray)) and len(smiles_batch) > 0:
                print(f"DEBUG: Adding {len(smiles_batch)} results to sampled_smiles")
                sampled_smiles.extend(smiles_batch)
                log_lhs.extend(log_lhs_batch)
                target_smiles.extend(batch["target_smiles"])
            else:
                print("DEBUG: No results from this batch")

        print(f"DEBUG: Predict completed with {len(sampled_smiles)} total results")
        print(f"DEBUG: Target SMILES count: {len(target_smiles)}")
